...
## Подготовка базы поиска (сильно улучшает точность)

Поиск точнее, когда у заметок есть блок «Возможные вопросы на собеседовании».
Их можно сгенерировать автоматически, локально, один раз:

1. Установи **Ollama**: https://ollama.com/download
2. Закрой Obsidian (чтобы освободить видеопамять под модель).
3. Двойной клик на **`prepare_base.bat`** (в папке плагина). Скачает модель
   `qwen2.5:7b-instruct` (~4.7 ГБ) и допишет вопросы во все заметки без блока
   (готовые/ручные не трогает — идемпотентно).
4. Открой Obsidian → нажми **Reindex** в панели плагина.

Тот же локальный движок (Ollama) на лету вытаскивает вопрос из разговора перед
поиском. Без Ollama плагин тоже работает (распознавание + поиск), просто без
этих двух улучшений.
