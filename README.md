...
## Phase 3 — Speaker enrollment (опционально)

Если system audio loopback не работает (телефонный собес, in-person, нет Zoom), плагин может различать твой голос от чужого на микрофоне. Команда: **Voice Vault: Enroll my voice (30s)** или из консоли:

```js
await window.vvsEnroll(30)
```

Запишет 30с твоего голоса, сохранит embedding ECAPA-TDNN в `enrolled_user.npy`. Дальше mic-транскрипты получают `is_user: true/false` и плагин пропускает твою речь.

Стереть: `await window.vvsClearEnrollment()`.
